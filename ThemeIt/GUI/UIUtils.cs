// Modified from SamsamTS's original Find It mod, https://github.com/SamsamTS/CS-FindIt
// Copied from FindIt2 sources (GNU GPL v2.0) https://github.com/sway2020/FindIt2/blob/master/FindIt/GUI/UIUtils.cs
// Very minor changes were made (namespace, made class static, formatting...).
...
public static class UIUtils {
    ...
    public static UICheckBox CreateCheckBox(UIComponent parent) {
        ...
        sprite.atlas = UIUtils.GetAtlas("Ingame");
        sprite.spriteName = "ToggleBase";
        sprite.size = new Vector2(16f, 16f);
        sprite.relativePosition = Vector3.zero;

        checkBox.checkedBoxObject = sprite.AddUIComponent<UISprite>();
        ((UISprite) checkBox.checkedBoxObject).atlas = UIUtils.GetAtlas("Ingame");
    ...
    public static UIColorField CreateColorField(UIComponent parent) {
        ...
        if (UIUtils._colorFIeldTemplate == null) {
            ...
            UIUtils._colorFIeldTemplate = template.Find<UIColorField>("LineColor");
            if (UIUtils._colorFIeldTemplate == null) return null;
        }

        UIColorField colorField = UnityEngine.Object.Instantiate<GameObject>(UIUtils._colorFIeldTemplate.gameObject)
    ...
    public static UITextureAtlas GetAtlas(string name) {
        if (UIUtils._atlases == null || !UIUtils._atlases.ContainsKey(name)) {
            ...
            for (int i = 0; i < atlases.Length; i++) {
                if (!UIUtils._atlases.ContainsKey(atlases[i].name))
                    UIUtils._atlases.Add(atlases[i].name, atlases[i]);
            }
        }

        return UIUtils._atlases[name];
    }
}
